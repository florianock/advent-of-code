...
let followInstructionsTestMoreMoves () =
    let stack = [| [| "X"; "X" |]; [| "X"; "A" |]; [| "X"; "X"; "X"; "C"; "B"; "X"; "X" |] |]
    let instructions = seq { "move 1 from 2 to 1"; "move 3 from 3 to 2" }
    let actual = followInstructions false (instructions, stack)
    Assert.AreEqual(
...
let parseTest () =
    ...
    Assert.AreEqual(
        seq { "move 1 from 2 to 1"; "move 3 from 1 to 3"; "move 2 from 2 to 1"; "move 1 from 1 to 2" },
        actualInstructions)
    Assert.AreEqual(
        [| [| "Z"; "N" |]; [| "M"; "C"; "D" |]; [| "P" |] |],
        actualStack)
