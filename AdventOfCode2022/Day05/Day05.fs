...
let setupStack (stacks: Stacks) (instruction: string): Stacks =
    [| for i = 0 to stacks.Length - 1 do
        if instruction[4 * i + 1] = ' ' then yield stacks[i]
        else yield Array.append [| instruction[4 * i + 1].ToString() |] stacks[i] |]
...
let getTopCrates (stacks: Stacks): string = stacks |> Array.map Array.last |> Array.fold (fun a b -> a + b) ""
